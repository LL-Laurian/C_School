...
void related_ingredients(char source_ingredient[MAX_STR_LEN])
{
     ...
     * Implement this function
     * ********/
    int j= ingredient_index(source_ingredient);
    if (j==-1){
        return;
    }
...
/*intNode *find_neigbour(intNode *source_head, char source_ingredient[MAX_STR_LEN], int k, int dist){
    if (dist>=k){
        return source_head;
    }
    int j= ingredient_index(source_ingredient);
    ...
    for (int i=0;i<MAT_SIZE;i++) {
        if (AdjMat[i][j] > 0) {
            if (searchInt(source_head,i)==0) {
                source_head = insertInt(source_head, i);
            }
            source_head=find_neigbour(source_head, ingredients[i], k, dist + 1);
        }
    }
    return source_head;
}
...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source, int k_avoid)
{
    ...
    if (avoid_index!=-1) {
        ...
        temp = source_head;
        while (temp != NULL) {
            if (searchInt(avoid_head, temp->x) == 0) {
                related = insertInt(related, temp->x);
            }
            temp=temp->next;
        }
...
int recipe_index(char recipe[10][MAX_STR_LEN], char to_change[MAX_STR_LEN])
{
    int i;
    for (i=0;i<10;i++){
...
int count_neigbour(char recipe[10][MAX_STR_LEN], int ingredient2_index, int change_index){
    int count=0;
    int i, recipe_index;
    for (i=0; i<10;i++){
        if (i==change_index){
        ...
        if(AdjMat[recipe_index][ingredient2_index]>0){
            count = count+AdjMat[recipe_index][ingredient2_index];
        }
    }
    return count;
}
...
void substitute_ingredient(char recipe[10][MAX_STR_LEN], char to_change[MAX_STR_LEN])
{
   ...
   */
    ...
    * Complete this function!
    ******/
    int change_index= recipe_index(recipe,to_change);
    ...
    count=0;
    for (i=0; i<MAT_SIZE;i++){
        if (count< count_neigbour(recipe, i, change_index)){
            count=count_neigbour(recipe, i, change_index);
            strcpy(recipe[change_index], ingredients[i]);
        }
    }
}
