...
BST_Node *newBST_Node(double freq, int bar, double index)
{
    ...
    //if (index<-0.999999 || index>=1.0){
        //printf("special: %f, %d, %f\n",freq,bar,index);
        //return NULL;
    //}
    //if (freq<16.35 || freq>4978.03){
        //return NULL;
    //}
...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
    ...
    if (key==root->key){
        if (root->left==NULL && root->right==NULL){
            ...
            node=*tmp;
            BST_delete(root->right, tmp->bar, tmp->index);
            root->key=node.key;
...
BST_Node *newBST_Node_reverse(double freq, int bar, double index)
{
    BST_Node *new_node=NULL;
...
BST_Node *Build_new_tree(BST_Node *root, BST_Node *new_root, int max, int min) {

    if (root==NULL){
        return NULL;
    }

    BST_Node *new_node = newBST_Node_reverse(root->freq, max - (root->bar) + min, 1.0 - (root->index));

    if (new_root == NULL) {
        new_root = BST_insert(new_root, new_node);
    } else {
        BST_insert(new_root, new_node);
    }

    Build_new_tree(root->left, new_root, max, min);
    Build_new_tree(root->right, new_root, max, min);
...
BST_Node *reverseSong(BST_Node *root)
{
    ...
    min=temp->bar;
    root= Build_new_tree(root, new_root, max, min);
    //MIN_BAR=0;
    //MIN_BAR=0;
    return root;
}
...
//BST_Node *another_root=NULL;

BST_Node *New_tree(BST_Node *root, BST_Node *another_root, int semitones, double time_shift){
    ...
    BST_Node *new_node=NULL;
    int index_of_freq=find_index(root->freq);
    if ((index_of_freq+semitones)>0 && (index_of_freq+semitones)<99 && ((root->index+time_shift)>0) && ((root->index+time_shift)<1)){
        new_node= newBST_Node(note_freq[index_of_freq+semitones],root->bar,root->index+time_shift);
        if (another_root==NULL){
            another_root=BST_insert(another_root, new_node);
        }
        else{
            BST_insert(another_root, new_node);
        }
    }
    else{
        new_node= newBST_Node(root->freq,root->bar,root->index);
        if (another_root==NULL){
            another_root=BST_insert(another_root, new_node);
        }
        else{
    ...
    New_tree(root->left, another_root, semitones,time_shift);
    New_tree(root->right, another_root,semitones,time_shift);

    return another_root;
}

BST_Node *insert_tree(BST_Node *root, BST_Node *second_root){

    if (second_root==NULL){
        return NULL;
    }
    BST_Node * new_insert= newBST_Node(second_root->freq,second_root->bar,second_root->index);
    insert_tree(root, second_root->left);
    BST_insert(root, new_insert);
    insert_tree(root, second_root->right);
    return root;
}
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
    ...
    printf("original root count: %d\n", count);
    BST_Node *another_root=NULL;
    if (New_tree(root,another_root, semitones,time_shift) == NULL){
        return root;
    }
    another_root=New_tree(root,another_root, semitones,time_shift);

    count = count_nodes(another_root);
    printf("another root count: %d\n", count);
    root = insert_tree(root, another_root);

    count = count_nodes(root);
    printf("new root count: %d\n", count);

    delete_BST(another_root);
